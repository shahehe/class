...
#import "LetterIdentificationD.h"
#import "CCAnimation+Helper.h"
#import "AnimatedSprite.h"

@implementation LetterIdentificationD
{
    // objects , weak ref
...
- (void) setEggWithIndex:(NSUInteger)idx
{
    if (idx >= frameNames.count)
        return;
    
    CGSize size = [[CCDirector sharedDirector] winSize];
    CGPoint size_p = ccpFromSize(size);
    
    NSString *name = [frameNames objectAtIndex:idx];
    ...
    {
        egg = [CCSprite spriteWithSpriteFrame:f];
        egg.zOrder = -2;
        [self addChild:egg];
    }
    
    CGPoint pos = CGPointFromString([framePositions objectForKey:name]);
    egg.position = ccpCompMult(pos, size_p);
}
...
- (id) initWithInitOptions:(NSDictionary *)options
{
    ...
    {
		[[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"eggs.plist"];
        frameNames = @[@"1.png",@"2.png",@"6.png",@"3.png",@"4.png"];
        [frameNames retain];
        
        framePositions = @{@"1.png": @"{0.515,0.436}",
                           @"2.png": @"{0.519,0.438}",
                           @"6.png": @"{0.518,0.396}",
                           @"3.png": @"{0.516,0.445}",
                           @"4.png": @"{0.501,0.441}"};
        [framePositions retain];
        
        [self setEggWithIndex:0];
        ...
        if (_numberOfScenes == 8 ) // has the sentence "here is a clue"
        {
            ...
            SceneBlock scene2 = ^(){
                ...
                id moveUp = [CCMoveBy actionWithDuration:2 position:ccp(0,100)];
                id moveDown = [CCMoveBy actionWithDuration:2 position:ccp(0,-100)];
                
                
                id myCallFunc2 = [CCCallFunc actionWithTarget:self selector:@selector(setFrame2)]; // to call our method
                id myCallFunc3 = [CCCallFunc actionWithTarget:self selector:@selector(setFrame3)]; // to call our method
                CCEaseBounceInOut *ease =
                [ CCEaseBounceInOut actionWithAction:[CCSequence actions:moveUp,myCallFunc2, moveDown,myCallFunc3,nil]];
                [egg runAction:ease];
        ...
        SceneBlock scene3 = ^(){
            ...
            CCSpriteFrame *f = [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"4.png"];
            egg.displayFrame = f;
        ...
        SceneBlock scene7 = ^(){
            ...
            id action3 = [CCFadeOut actionWithDuration:1];
            id action4 = [CCDelayTime actionWithDuration:1];
            id seq = [CCSequence actions:action1,action2,action3,action4,nil];
            id repeat = [CCRepeatForever actionWithAction:seq];
