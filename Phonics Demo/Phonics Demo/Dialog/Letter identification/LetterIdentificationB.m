...
#import "LetterIdentificationB.h"
#import "CCAnimation+Helper.h"
#import "AnimatedSprite.h"

@implementation LetterIdentificationB
{
    // objects , weak ref
    ...
    AnimatedSprite * flyingButterfly;
...
- (id) initWithInitOptions:(NSDictionary *)options
{
    if (self = [super initWithInitOptions:options])
    {
        ...
        letterY = [CCSprite spriteWithFile:@"yy.png"];
        letterYY = [CCSprite spriteWithFile:@"y.png"];
        letterY.position = CCMP(0.4,0.4);
        letterYY.position = CCMP(0.4,0.4);
        
        [whiteboard addChild:letterYY];
        [whiteboard addChild:letterY];
        letterYY.visible = NO;
        letterY.visible = NO;
        
        letterPic = [CCSprite spriteWithFile:@"iPICTURE.png"];
        letterWrite = [CCSprite spriteWithFile:@"iLETTER.png"];
        letterPic.position = CCMP(0.4,0.4);
        letterWrite.position = CCMP(0.4,0.4);
        [whiteboard addChild:letterPic];
        [whiteboard addChild:letterWrite];
        letterWrite.visible = NO;
        letterPic.visible = NO;
        ...
        SceneBlock scene1 = ^(){
            ...
            NSString *spriteSheetPrefix = @"DialogSoundA";
            ...
            flyingButterfly = [AnimatedSprite spriteWithSpriteFrameName:[NSString stringWithFormat:@"butterfly%04d.png",1]] ;
            [spriteSheet addChild:flyingButterfly];
            [flyingButterfly addLoopingAnimation:@"fly" frame:@"butterfly%04d.png" delay:0.3 firstFrameIdx:1];
            
            [spriteSheet setPosition:CCMP(0.1,0.05)];
        ...
        if (_numberOfScenes == 8 ) // has the sentence "here is a clue"
        {
            ...
            SceneBlock scene2 = ^(){
                [flyingButterfly setPosition:CCMP(0.1, 0.05)];
                [flyingButterfly startAnimation:@"fly"];
                
                ccBezierConfig bezier;
                
                // bu.position = CCMP(0.1,0.05);
                bezier.controlPoint_1 = CCMP(0.2,0.2);
                //bezier.controlPoint_2 = CGPointMake(260, 290.0f);
                bezier.endPosition =CCMP(0.6,0.1);
                
                id bezierForward = [CCBezierBy actionWithDuration:3 bezier:bezier];
                [flyingButterfly runAction: [CCSequence actions:
                                             bezierForward,
                                             nil]];
        ...
        SceneBlock scene4 = ^(){
            ...
            [flyingButterfly stopAnimation:@"fly"];
        ...
        SceneBlock scene5 = ^(){
            ...
            //letterWrite.scale = 0.2;
            //letterPic.scale = 0.2;
            letterWrite.visible = YES;
            letterPic.visible = YES;
            
            id action1 = [CCFadeIn actionWithDuration:2];
            id action2 = [CCDelayTime actionWithDuration:3];
            id action3 = [CCFadeOut actionWithDuration:2];
